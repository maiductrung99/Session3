...
class DataReader:
    def __init__(self, data_path, batch_size, vocab_size):
        ...
        for data_id, line in enumerate(d_lines):
            ...
            for token in tokens:
                ind, value= int(token.split(":")[0]), float(token.split(":")[1])
                tfidf_vec[ind]= value
    ...
    def next_batch(self):
        ...
        if end + self._batch_size > len(self._data):
            ...
            self._data, self._label= self._data[indices], self._labels[indices]
...
class MLP:
    ...
    def build_graph(self):
        ...
        #set up hidden theo ham sigmoid
    ...
    def trainer(self, loss,learning_rate):
        ...
        return train_op
def load_dataset():
    #doc DL theo ham train va test, ghi DL vao 2 bien va tra ve 2 bien do
    ...
    test_data_reader= DataReader(
        data_path='./datasets/20news-bydate/20new-test-tfidf.txt',
        batch_size=50,
        vocab_size=vocab_size
    )
...
def save_parameters(name, value, epoch):
    ...
    if len(value.shape) == 1:
        string_form=','.join([str(nummber) for number in value])
    else:
        string_form= '\n'.join([','.join([str(number)
                                         for number in value[row]])
                                for row in range(value.shape[0])])
    with open('./datasets/20news-bydate/'+ filename, 'w') as f:
        f.write(string_form)
...
def train_model():    
    ...
    with tf.Session() as sess:
        ...
        while step < MAX_STEP:
            train_data, train_labels= train_data_render.next_batch()
...
def test_model():
    ...
    with tf.Session() as sess:
        ...
        num_true_preds=0
        while True:
            test_data, test_labels= test_data_render.next_batch()
